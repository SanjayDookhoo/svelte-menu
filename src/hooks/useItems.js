	...
	const dispatch = useCallback(
		(actionType, item, nextIndex) => {
			...
			const sortItems = () => {
				...
				const orderedNodes =
					menuRef.current.querySelectorAll('.svelte-menu__item');
				items.sort(
					(a, b) => indexOfNode(orderedNodes, a) - indexOfNode(orderedNodes, b)
				);
			...
			switch (actionType) {
				...
				default:
					if (process.env.NODE_ENV !== 'production')
						throw new Error(
							`[React-Menu] Unknown hover action type: ${actionType}`
						);
			}
